({getBrandsList: function (component, event, helper) {
    ...
    getCountByBrands: function (component, event, helper) {
        var action = component.get("c.getCountByBrands");
        action.setCallback(this, function (response) {
                var state = response.getState();
                if (state === "SUCCESS"){
                    component.set("v.brandCountList", response.getReturnValue());
                }});
            $A.enqueueAction(action);
    },
    ...
    getSelectedProducts: function (component, event, helper) {
        ...
        var Brand = event.getSource().get("v.name");
